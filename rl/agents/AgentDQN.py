...
from rl.agents.net import QNet, QNetDuel
from rl.agents.net import QNetTwin, QNetTwinDuel
from rl.train.config import Config
from rl.train.replay_buffer import ReplayBuffer
...
class AgentDQN(AgentBase):
    ...
    def explore_one_env(self, env, horizon_len: int, if_random: bool = False) -> Tuple[Tensor, ...]:
        ...
        masks = torch.zeros((horizon_len, self.num_envs, self.action_dim), dtype=torch.float32).to(self.device)
        ...
        for t in range(horizon_len):
            action = torch.randint(self.action_dim, size=(1, 1)) if if_random else get_action(state, mask)  # different
            states[t] = state
            masks[t] = mask

            ary_action = action[0, 0].detach().cpu().numpy()
            ary_state, reward, done, ary_mask = env.step(ary_action)  # next_state
            if done:
                ary_state, ary_mask = env.reset()  # ary_state.shape == (state_dim, )
            state = torch.as_tensor(ary_state, dtype=torch.float32, device=self.device).unsqueeze(0)
            mask = torch.as_tensor(ary_mask, dtype=torch.float32, device=self.device).unsqueeze(0)
            actions[t] = action
            rewards[t] = reward
            dones[t] = done
        ...
        return states, actions, rewards, undones, masks
    ...
    def explore_vec_env(self, env, horizon_len: int, if_random: bool = False) -> Tuple[Tensor, ...]:
        ...
        rewards = torch.zeros((horizon_len, self.num_envs), dtype=torch.float32).to(self.device)
        dones = torch.zeros((horizon_len, self.num_envs), dtype=torch.bool).to(self.device)
        masks = torch.zeros((horizon_len, self.num_envs), dtype=torch.float32).to(self.device)
        ...
        for t in range(horizon_len):
            ...
            state, reward, done, mask = env.step(action)  # next_state
            actions[t] = action
            rewards[t] = reward
            dones[t] = done
            masks[t] = mask
    ...
    def update_net(self, buffer: ReplayBuffer) -> Tuple[float, ...]:
        with torch.no_grad():
            states, actions, rewards, undones, masks = buffer.add_item
    ...
    def get_obj_critic_raw(self, buffer: ReplayBuffer, batch_size: int) -> Tuple[Tensor, Tensor]:
        ...
        """
        with torch.no_grad():
            states, actions, rewards, undones, next_ss, masks = buffer.sample(batch_size)  # next_ss: next states
            cri_target = self.cri_target(next_ss) + (1 - masks) * -99999
            next_qs = cri_target.max(dim=1, keepdim=True)[0].squeeze(1)  # next q_values
