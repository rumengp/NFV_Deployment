...
def get_cumulative_rewards_and_steps(env, actor, if_render: bool = False) -> Tuple[float, int]:
    ...
    for steps in range(max_step):
        ...
        tensor_mask = torch.as_tensor(mask, dtype=torch.float32, device=device).unsqueeze(0)
        tensor_action = actor(tensor_state)
        if if_discrete:
            tensor_probs = (1 - tensor_mask) * -9999 + tensor_action
            tensor_action = tensor_probs.argmax(dim=1)
...
def demo_evaluator_actor_pth():
    import gym
    from rl.agents.AgentPPO import AgentPPO
    from rl.train.config import Config, build_env
...
def demo_evaluate_actors(dir_path: str, gpu_id: int, agent, env_args: dict, eval_times=2, net_dim=128):
    import gym
    from rl.train.config import build_env
...
def demo_load_pendulum_and_render():
    ...
    from rl.envs.CustomGymEnv import PendulumEnv
    ...
    # from rl.train.evaluator import get_cumulative_returns_and_step
...
def run():
    ...
    title = f"{env_name}_{agent.__name__}_rl"
