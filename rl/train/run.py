...
from rl.train.evaluator import Evaluator, get_cumulative_rewards_and_steps
...
def train_agent(args: Config):
    ...
    if args.num_envs == 1:
        ...
        mask = torch.tensor(mask, dtype=torch.float32, device=agent.device).unsqueeze(0)
    else:
    ...
    if args.if_off_policy:
        buffer = ReplayBuffer(
            ...
            mask_dim = args.action_dim,
            action_dim=1 if args.if_discrete else args.action_dim,
            if_use_per=args.if_use_per,
            args=args,
        )
        buffer_items = agent.explore_env(env, args.horizon_len * args.eval_times, if_random=False)
